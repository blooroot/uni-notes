...
void pantalla() {
    ...
    for (float theta = 0; theta <= 4 * M_PI; theta += 0.01f) {
        // we're adding the rotation angle to theta: (-170.0 * M_PI / 180.0)
        float r = 6 * (cos((5/1) * theta + (-170.0 * M_PI / 180.0))); // Polar rose equation (n/d, n=2, d=1)
        float x = r * cos(theta);
        float y = r * sin(theta);
        glVertex2f(x, y);
    }
...
int main(int argc, char *argv[]) {
    ...
    glutCreateWindow("Thin Polar Rose"); // New window name
